...
Theme integrations can be modified by a theme user from inside the Astro config.
Theme authors define the shape for this user configuration using a [Zod](https://zod.dev/) schema.
...
The user configuration can customize pages and components inside the theme:
...
The theme config must be JSON serializable. Values like functions and classes do not work inside theme configurations.
