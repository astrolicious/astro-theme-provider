...
The default style module can be overridden using the `imports` option.
...
Elements inside themes should always include a class, even if it is not being styled, so that users have an option to target the element for style changes.
Themes should choose class names that include a prefix to prevent collision with existing styles:
...
Astro Theme Provider cannot guarantee the order of styles loaded from tags which can lead to issues when users try to override the styles.
Instead, styles should be authored inside of separate files and consumed using [virtual style modules](#virtual-style-modules)
